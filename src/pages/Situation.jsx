...
const pageText = {
  FR: {
    ...
    recto: "Résultats",
    verso: "Configuration",
    flipHint: "Touchez pour voir l’explication",
    backHint: "Touchez pour revenir aux résultats",
    totalSaved: "Total déjà mis de côté",
    totalIncome: "Total des revenus",
    totalExpenses: "Total des sorties",
    ownedNow: "Possédé actuellement",
    targetGoal: "Objectif conseillé",
    emergencyExplain:
      "Le fonds d’urgence représente l’argent déjà disponible pour absorber les imprévus sans créer de nouvelle dette.",
    projectsExplain:
      "L’épargne projets représente l’argent déjà mis de côté pour maison, voyage, auto, études, entreprise ou autre projet futur.",
    savingsNote:
      "Ici, vous entrez ce que vous possédez déjà. Les montants visés se configurent dans Objectifs ou Simulateur.",
    incomeNote:
      "Cette carte indique d’où vient l’argent qui alimente votre plan mensuel.",
    expensesNote:
      "Cette carte indique où part votre argent chaque mois : logement, transport, nourriture, assurances et autres sorties.",
    configureHere: "Configurer ici les montants actuels",
  ...
  EN: {
    ...
    recto: "Results",
    verso: "Configuration",
    flipHint: "Tap to see explanation",
    backHint: "Tap to return to results",
    totalSaved: "Total already set aside",
    totalIncome: "Total income",
    totalExpenses: "Total outgoing",
    ownedNow: "Currently owned",
    targetGoal: "Suggested target",
    emergencyExplain:
      "The emergency fund is money already available to absorb surprises without creating new debt.",
    projectsExplain:
      "Project savings is money already set aside for home, travel, car, studies, business or another future project.",
    savingsNote:
      "Here, you enter what you already own. Target amounts are configured in Goals or Simulator.",
    incomeNote:
      "This card shows where the money funding your monthly plan comes from.",
    expensesNote:
      "This card shows where your money goes every month: housing, transport, food, insurance and other outflows.",
    configureHere: "Configure current amounts here",
    progress: "Progress",
  },
  ES: {
    ...
    recto: "Resultados",
    verso: "Configuración",
    flipHint: "Toca para ver explicación",
    backHint: "Toca para volver a resultados",
    totalSaved: "Total ya reservado",
    totalIncome: "Total de ingresos",
    totalExpenses: "Total de salidas",
    ownedNow: "Poseído actualmente",
    targetGoal: "Objetivo sugerido",
    emergencyExplain:
      "El fondo de emergencia es dinero ya disponible para absorber imprevistos sin crear nueva deuda.",
    projectsExplain:
      "El ahorro para proyectos es dinero ya reservado para casa, viaje, auto, estudios, empresa u otro proyecto futuro.",
    savingsNote:
      "Aquí ingresas lo que ya posees. Los objetivos se configuran en Objetivos o Simulador.",
    incomeNote:
      "Esta tarjeta muestra de dónde viene el dinero que alimenta tu plan mensual.",
    expensesNote:
      "Esta tarjeta muestra a dónde va tu dinero cada mes: vivienda, transporte, comida, seguros y otras salidas.",
    configureHere: "Configurar aquí los montos actuales",
    progress: "Progreso",
  },
};
...
function Situation({ financeData, setFinanceData, settings, setCurrentPage }) {
  ...
  const [details, setDetails] = useState(() => {
    try {
      return (
        ...
        getDefaultDetails(language)
      );
    } catch {
      return getDefaultDetails(language);
    }
  });
  ...
  const emergencyAmount =
    details.savings.find((item) => item.id === "emergency")?.amount || 0;
  const projectSavingsAmount =
    details.savings.find((item) => item.id === "projects")?.amount || 0;
  const emergencyTarget = Math.max(1000, Math.round(expensesTotal * 3 || 3000));
  const projectsTarget = Math.max(1000, Math.round(projectSavingsAmount * 2 || 10000));
  ...
  return (
    <div className="native-page">
      ...
      <FlipCategorySection
        ...
        total={incomeTotal}
        totalLabel={p.totalIncome}
        currency={currency}
        flipped={flippedCards.incomes}
        note={p.incomeNote}
        color="var(--green)"
        onFlip={() => toggleFlip("incomes")}
      ...
      <FlipCategorySection
        ...
        total={expensesTotal}
        totalLabel={p.totalExpenses}
        currency={currency}
        flipped={flippedCards.expenses}
        note={p.expensesNote}
        color="var(--gold)"
        onFlip={() => toggleFlip("expenses")}
      ...
      <FlipCategorySection
        ...
        total={savingsTotal}
        totalLabel={p.totalSaved}
        currency={currency}
        flipped={flippedCards.savings}
        note={p.savingsNote}
        color="var(--green)"
        savingsTargets={{
          emergency: emergencyTarget,
          projects: projectsTarget,
          emergencyExplain: p.emergencyExplain,
          projectsExplain: p.projectsExplain,
          ownedNow: p.ownedNow,
          targetGoal: p.targetGoal,
          progress: p.progress,
        }}
        onFlip={() => toggleFlip("savings")}
      ...
      <section
        id="dettes"
        style={card}
        className={confirmAction === "debt" ? "action-confirm" : ""}
      >
        <div style={header}>
        ...
        {safeFinanceData.debts.map((debt, index) => (
          <div key={`${debt.name}-${index}`} style={debtCard}>
            ...
            <input
              value={debt.balance || ""}
              onChange={(event) => updateDebt(index, "balance", event.target.value)}
              style={input}
              inputMode="decimal"
      ...
      >
        ...
        <select
          value={newDebt.type}
          onChange={(event) => setNewDebt({ ...newDebt, type: event.target.value })}
          style={input}
        >
          <option>Carte de crédit</option>
          <option>Marge de crédit</option>
        ...
        <input
          ...
          onChange={(event) =>
            setNewDebt({ ...newDebt, balance: cleanMoneyInput(event.target.value) })
          }
          style={input}
          inputMode="decimal"
          placeholder={p.balance}
      ...
      <section style={heroCard}>
        <div style={heroTop}>
          <div>
            ...
            <h2 style={scoreTitle}>
              {score.isReady ? `${score.value}/100` : p.scoreWaiting}
            </h2>
        ...
        <div style={barBg}>
          <div
            style={{
              ...barFill,
              width: `${score.isReady ? score.value : 0}%`,
      ...
      <section style={priorityCard}>
        ...
        <div style={actions}>
          ...
          <button onClick={() => setCurrentPage("simulateur")} style={simulatorButton}>
            {p.openSimulator}
          </button>
        </div>
      </section>
...
function FlipCategorySection({
  ...
  return (
    <section id={id} style={{ ...flipCard, borderColor: flipped ? color : "var(--border)" }}>
      <button onClick={onFlip} style={flipHeader} type="button">
        <div style={headerLeft}>
          {icon}
          <div>
            <h2>{title}</h2>
            <p style={mutedSmall}>
              {flipped ? "Configuration / explication" : "Résultats actuels"}
            </p>
          </div>
        </div>

        <span style={{ ...flipPill, borderColor: color, color }}>
          {flipped ? "Verso" : "Recto"}
          <RotateCcw size={14} />
        </span>
      </button>
      ...
      {!flipped ? (
        <div style={flipBody}>
          <div style={resultHero}>
            <span style={mutedSmall}>{totalLabel}</span>
            <strong>{formatMoney(total, currency)}</strong>
          </div>

          {savingsTargets ? (
            <div style={savingsGrid}>
              {items.map((item) => {
                const target =
                  item.id === "emergency"
                    ? savingsTargets.emergency
                    : savingsTargets.projects;
                const progress = percentage(item.amount, target);

                return (
                  <div key={item.id} style={savingResult}>
                    <div style={metricHead}>
                      <strong>{item.label}</strong>
                      <span>{progress}%</span>
                    </div>
                    <p style={mutedTiny}>
                      {savingsTargets.ownedNow} : {formatMoney(item.amount, currency)}
                    </p>
                    <p style={mutedTiny}>
                      {savingsTargets.targetGoal} : {formatMoney(target, currency)}
                    </p>
                    <div style={miniBarBg}>
                      <div
                        style={{
        ...
        <div style={flipBody}>
          ...
          {savingsTargets && (
            <>
              <InfoBlock
                title={items.find((item) => item.id === "emergency")?.label}
                text={savingsTargets.emergencyExplain}
              />
              <InfoBlock
                title={items.find((item) => item.id === "projects")?.label}
                text={savingsTargets.projectsExplain}
              />
            </>
          )}

          <p style={sectionLabel}>Configurer ici les montants actuels</p>
...
function getPremiumStatus(score, available, p) {
  if (!score.isReady) {
    return {
      label: p.scoreWaiting,
      color: "var(--gold)",
      icon: <AlertTriangle color="var(--gold)" />,
    };
  }

  if (score.value >= 75 && available >= 0) {
    return {
      label: p.positive,
      color: "var(--green)",
...
function calculateScore({ income, expenses, savings, totalDebt, available, p }) {
  ...
  if (value >= 80) return { value, isReady: true, label: p.good, color: "var(--green)" };
  if (value >= 60) return { value, isReady: true, label: p.stable, color: "var(--gold)" };
  if (value >= 40) return { value, isReady: true, label: p.fragile, color: "var(--red)" };

  return { value, isReady: true, label: p.control, color: "var(--red)" };
}
...
const heroCard = {
  background:
    "radial-gradient(circle at top right, rgba(212,175,55,.20), transparent 34%), linear-gradient(135deg, rgba(34,197,94,.12), var(--bg-card))",
...
const speCard = {
  background:
    "radial-gradient(circle at top right, rgba(212,175,55,.22), transparent 36%), linear-gradient(135deg, rgba(212,175,55,.14), rgba(56,189,248,.08), var(--bg-card))",
